...
import type { IStorageData } from "../common/types";
import { IEnvironmentService, IStorageService } from "../common/types";
import { ILogService } from "@/logger/common";
import type { IDisposable } from "@opensumi/ide-core-common";
import type { ElectronMainApiRegistry } from "@opensumi/ide-core-electron-main/lib/bootstrap/types";

@Injectable()
export class StorageService
  extends ElectronMainApiProvider
  implements IStorageService, IDisposable
{
  ...
  private async doInit() {
    try {
      await fs.mkdir(path.dirname(this.environmentService.storagePath), {
        recursive: true,
      });
      this.#lastContents = await fs.readFile(
        this.environmentService.storagePath,
        "utf8",
      );

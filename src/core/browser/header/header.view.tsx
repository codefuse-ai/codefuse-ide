...
const useMaximize = () => {
  const uiService: IElectronMainUIService = useInjectable(
    IElectronMainUIService,
  );

  const [maximized, setMaximized] = useState(false);

  const getMaximized = async () =>
    uiService.isMaximized(electronEnv.currentWindowId);

  useEffect(() => {
    const maximizeListener = uiService.on(
      "maximizeStatusChange",
...
export const ElectronHeaderBar = () => {
  ...
  const layoutViewSize =
    useInjectable<LayoutViewSizeConfig>(LayoutViewSizeConfig);
...
export const HeaderBarTitleComponent = () => {
  const headerService = useInjectable(
    IElectronHeaderService,
  ) as IElectronHeaderService;
  ...
  function setPosition() {
    window.requestAnimationFrame(() => {
      if (ref.current && spanRef.current) {
        ...
        const leftWidth =
          menuBarLeftWidth + extraWidth + (isMacintosh ? macTrafficWidth : 0);
        const left = Math.max(
          0,
          windowWidth * 0.5 - leftWidth - spanRef.current.offsetWidth * 0.5,
        );

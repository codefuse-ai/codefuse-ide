...
import type { IUpdateMainService } from "../common";
  ...
  implements IUpdateMainService
{
  ...
  async checkForUpdatesAuto(): Promise<void> {
    ...
    const loopCheck = (first = true) => {
      this.#checkTimer = setTimeout(
        () => {
          this.checkForUpdates({ manual: false }).then(() => {
            loopCheck(false);
          });
        },
        (first ? 10 : 3600) * 1000,
  ...
  async checkForUpdates({ manual = false }) {
    ...
    this.logger.debug(
      `[auto-updater] checkForUpdates: ${manual ? "手动更新" : "自动更新"}, current updateState: ${updateState}`,
    );
    ...
    switch (updateState) {
      case UpdateState.Available:
        ...
        if (
          manual ||
          !this.updaterService.ignoreVersion.has(
            // eslint-disable-next-line @typescript-eslint/no-non-null-asserted-optional-chain
            this.updaterService.updateInfo?.version!,
          )
        ) {

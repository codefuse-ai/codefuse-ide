...
import { AI_MENU_BAR_LEFT_ACTION, AIFeatureModule } from "@/ai/browser";
...
terminalPreferenceSchema.properties[
...
async function renderApp() {
  const opts: IClientAppOpts = {
    ...
    treeSitterWasmDirectoryUri: URI.file(
      electronEnv.treeSitterWasmDirectoryPath,
    ).toString(true),
    AINativeConfig: {
      layout: {
        menubarLogo: logo,
  ...
  app.fireOnReload = () => {
    app.injector
      ...
      .reloadWindow(electronEnv.currentWindowId);
  };

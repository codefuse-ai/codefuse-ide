...
function createRPCNetConnection() {
  const rpcListenPath = ipcRenderer.sendSync(
    "window-rpc-listen-path",
    electronEnv.currentWindowId,
  );
...
const metaData = JSON.parse(
  ipcRenderer.sendSync("window-metadata", electronEnv.currentWindowId),
);
electronEnv.metadata = metaData;
process.env = Object.assign({}, process.env, metaData.env, {
  WORKSPACE_DIR: metaData.workspace,
});

electronEnv.onigWasmPath = path.join(
  __dirname,
  "..",
  "..",
  "..",
  metaData.environment.isDev ? "node_modules" : "node_modules.asar.unpacked",
  "vscode-oniguruma/release/onig.wasm",
);
electronEnv.treeSitterWasmDirectoryPath = path.join(
  __dirname,
  "..",
  "..",
  "..",

...
const shellIntegrationDirPath = path.join(
  os.homedir(),
  process.env.IDE_DATA_FOLDER_NAME!,
  "shell-integration",
);

export const bashIntegrationPath = path.join(
  shellIntegrationDirPath,
...
export class ShellIntegrationService extends BaseShellIntegrationService {
  ...
  async initBashInitFile(): Promise<string> {
    ...
    await fs.writeFile(
      bashIntegrationPath,
      await this.getBashIntegrationContent(),
    );

...
import { ChatCompletionRequestMessageRoleEnum } from "@opensumi/ide-ai-native/lib/common";
...
import type { ChatCompletion, ChatCompletionChunk, Completion } from "./types";
import type {
  IAIBackService,
...
import type { CancellationToken, INodeLogger } from "@opensumi/ide-core-node";
import type { fetch as FetchType, Response } from "undici-types";
...
export class AIBackService extends BaseAIBackService implements IAIBackService {
  ...
  override async request(
    ...
    if (!response.ok) {
      this.logger.error(
        `ai request failed: status: ${response.status}, body: ${await response.text()}`,
      );
      return {
        errorCode: 1,
  ...
  override async requestStream(
    ...
    if (!response.ok) {
      this.logger.error(
        `ai request stream failed: status: ${response.status}, body: ${await response.text()}`,
      );
  ...
  async requestCompletion(
          ...
          ? {
              messages: [
                  ...
                  : []),
                {
                  ...
                  content: config.codeFimTemplate
                    .replace("{prefix}", input.prompt)
    ...
    if (!response.ok) {
      this.logger.error(
        `ai request completion failed: status: ${response.status}, body: ${await response.text()}`,
      );
      return {
        sessionId: input.sessionId,
        codeModelList: [],
    ...
    try {
      ...
      const content = config.codeFimTemplate
        ? (data as ChatCompletion)?.choices?.[0]?.message?.content
        : (data as Completion)?.choices?.[0]?.text;
      if (!content) {
        return {
          sessionId: input.sessionId,
          codeModelList: [],
      ...
      this.logger.error(
        `ai request completion body parse error: ${err?.message}`,
      );
      throw err;
    }
  }
  ...
  private checkConfig(isCodeCompletion = false) {
    ...
    const modelName = isCodeCompletion
      ? config.codeModelName || config.chatModelName
  ...
  private async fetchModel(
    ...
    return (fetch as typeof FetchType)(url, {
      signal,
      method: "POST",
      headers: {

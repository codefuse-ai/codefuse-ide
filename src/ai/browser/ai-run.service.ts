...
import type { IAIBackService, URI } from "@opensumi/ide-core-common";
import type { DebugConfiguration } from "@opensumi/ide-debug";
...
export class AiRunService {
  ...
  get pkgUri() {
    const workspaceFolderUri =
      this.workspaceService.getWorkspaceRootUri(undefined);
...
`;
    ...
    const reportRelationId = this.aiReporter.start("aiRunFrontEnd", {
      message: "aiRunFrontEnd",
    });
    ...
    if (!res.data || res.errorCode !== 0) {
      ...
      this.aiReporter.end(reportRelationId, {
        message: "aiRunFrontEndModelFetchError",
        success: false,
      });
      return undefined;
    }

...
export class AICommandService {
  ...
  constructor() {
    this.logger = this.loggerManagerClient.getLogger(
      SupportLogNamespace.Browser,
    );
  }

  async requestToModel(prompt: string) {
    return this.aiBackService.request<
      IAIBackServiceOption & { maxTokens: number }
  ...
  async classifyCommand() {
    ...
    if (unGroupCommands.length) {
      const partCommands = Array.from(
        {
          length:
            Math.round(unGroupCommands.length / this.commandRequestStep) || 1,
        },
        (_, index) => index,
      ).map((i) =>
        unGroupCommands.slice(
          i * this.commandRequestStep,
          (i + 1) * this.commandRequestStep,
        ),
      );
  ...
  async requestForClassifyCommand(commands: Command[]) {
    const prompt = this.promptManager.groupCommand(
  ...
  public async getModelResp(
    input: string,
  ): Promise<IAIBackServiceResponse<ISumiModelResp>> {
    /**
  ...
  public async searchCommand(
    input: string,
  ): Promise<IAIBackServiceResponse<Command>> {
  ...
  public async findCommand(
    ...
    const commands = this.commandRegistryService
      ...
      .filter(
        (c) =>
          commandsInGroup.includes(c.id) ||
          (c.delegate && commandsInGroup.includes(c.delegate)),
      );
    ...
    try {
      const command = await Promise.any(
        partCommands.map((c) => this.requestCommand(c, input)),
      );
  ...
  private searchWithoutAI(input: string) {
    return this.commandRegistryService
      ...
      .find(
        (command) =>
          command.labelLocalized?.localized === input ||
          command.label === input,
      );
  }
    ...
    const prompt = this.promptManager.findCommand({
      commands: commands
        .map(
          ...
          (c) => `{${c.id}}-{${c.labelLocalized?.localized! || c.label || ""}}`,
        )
        .join("\n"),
      question,
    });
